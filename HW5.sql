...
CREATE TABLE `codes` (
  ...
  `dialingCode` varchar(20) NOT NULL
) ENGINE=InnoDB DEFAULT CHARSET=latin1;


 /* Dumping data for table `codes`*/

INSERT INTO `codes` (`state`, `id`, `areaCode`, `dialingCode`) VALUES
('AL', 1, 334, '+1-334'),
('AK', 2, 907, '+1-907'),
('AZ', 3, 602, '+1-602'),
('AR', 4, 501, '+1-501'),
...
CREATE DEFINER=`admin`@`localhost` PROCEDURE `FixUSPhones` ()  BEGIN
...
DROP PROCEDURE IF EXISTS SetUSInternatiionalPhones;
CREATE DEFINER=`admin`@`localhost` PROCEDURE `SetUSInternatiionalPhones` ()  BEGIN
	DECLARE finished INTEGER DEFAULT 0;
	DECLARE phone varchar(50) DEFAULT "x";
	DECLARE customerNumber INT DEFAULT 0;
    DECLARE country varchar(50) DEFAULT "";
	DECLARE us_state varchar(50) DEFAULT "";
	DECLARE checked_state varchar(520) DEFAULT "";
	DECLARE area INT DEFAULT 0;
	DECLARE dcode varchar(20) DEFAULT "";
	...
	DECLARE curPhone
		CURSOR FOR 
            		SELECT customers.customerNumber, customers.phone, customers.country , customers.state
				FROM classicmodels.customers;


	-- declare cursor for codes and states
	Declare curCodes
		CURSOR FOR
					SELECT codes.state, codes.areaCode, codes.dialingCode
				FROM classicmodels.codes;
	...
	OPEN curCodes;
    	...
    	-- create a copy of the customer table 
	DROP TABLE IF EXISTS classicmodels.international_numbers;
	CREATE TABLE classicmodels.international_numbers LIKE classicmodels.fixed_customers;
	INSERT international_numbers SELECT * FROM classicmodels.fixed_customers;

	fixPhone: LOOP
		FETCH curPhone INTO customerNumber,phone, country, us_state;
		IF finished = 1 THEN 
			LEAVE fixPhone;
		END IF;


		FETCH curCodes INTO checked_state, area, dcode;
		IF finished = 1 THEN
			LEAVE fixPhone;
		END IF;
